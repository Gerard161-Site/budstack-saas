...
export function TenantEditTemplateClient({ template }: EditTemplateClientProps) {
    ...
    const handleSave = async (data: EmailTemplateData) => {
        setIsSaving(true);
        try {
            const res = await fetch(`/api/tenant-admin/email-templates/${template.id}`, {
                method: 'PUT',
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-6rem)]">
            <div className="px-6 py-4 border-b">
                <h1 className="text-2xl font-bold tracking-tight">Edit Custom Template</h1>
